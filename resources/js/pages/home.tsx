...
import { Film, Star, Users, TrendingUp, Award, ChevronRight, Eye } from 'lucide-react';
import { Navbar } from '@/components/navbar';
import { FilmCard } from '@/components/film-card';
import { Button } from '@/components/ui/button';

export default function Home() {
    const { auth, popularFilms = [], bestFilms = [] } = usePage<{
    ...
    return (
        <>
            <Head title="ulas.film — Platform Review Film" />
                    ...
                    <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center">
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                            <Button
                                asChild
                                size="lg"
                                className="bg-[#F5C518] text-[#0D0D0D] hover:bg-[#E5B500] font-semibold px-8"
                            >
                                <Link href="/film">
                                    Jelajahi Film
                            ...
                            {!auth.user && (
                                <Button
                                    ...
                                    variant="outline"
                                    size="lg"
                                    className="border-[#2D2D2D] text-[#B3B3B3] hover:text-white hover:border-[#3D3D3D] hover:bg-[#1A1A1A] px-8"
                                >
                        ...
                        <div className="grid grid-cols-3 gap-4 mt-16 max-w-xl mx-auto">
                            {[
                                { icon: Film, label: 'Film', value: popularFilms.length + bestFilms.length },
                                { icon: Users, label: 'Reviewer', value: '500+' },
                                { icon: Star, label: 'Rating', value: '2K+' },
